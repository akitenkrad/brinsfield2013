...
## Deferred: real-data Phase 3

The empirical Track A real-data path (independent-sample CFA on a collected survey) is the only deferred element. The tooling runs against it today via `survey-loader --csv <path>`; until a real instrument is administered, the calibrated `--synthesize-n` path provides the end-to-end smoke.
